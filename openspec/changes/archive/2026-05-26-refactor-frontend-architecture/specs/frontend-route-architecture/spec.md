...
The frontend SHALL scope Tutor Chat messages and card lookups to the active Module, Note Group, or Concept context.

#### Scenario: Open Tutor Chat in Module context
- **WHEN** a user opens Tutor Chat from a Module page
- **THEN** the chat request includes Module context and does not require a selected Note Group

#### Scenario: Open Tutor Chat in Note Group context
- **WHEN** a user opens Tutor Chat from a Note Group page
- **THEN** the chat request includes Note Group context

#### Scenario: Open Tutor Chat in Concept context
- **WHEN** a user opens Tutor Chat from a Concept page
- **THEN** the chat request includes Concept context
...
- **THEN** the file primarily defines providers, the route tree, shell-level concerns, global dialogs, and toasts

#### Scenario: Feature ownership outside App
- **WHEN** a developer changes Module, Note Group, Concept, Review, Tutor Chat, or Auto Workflow behavior
